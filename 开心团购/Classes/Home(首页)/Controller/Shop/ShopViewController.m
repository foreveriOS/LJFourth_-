...
//        MTLog(@"%@",_shopInfoM);
//        MTLog(@"%@",_shopRecommendArray);
    });
}

- (void)initData{
    _shopInfoM = [[ShopInfoModel alloc] init];
    _shopRecommendArray = [NSMutableArray array];
}
...
//    [UMSocialSnsService presentSnsIconSheetView:self appKey:UMAPPKEY shareText:@"分享到三方" shareImage:[UIImage imageNamed:@"m1.jpg"] shareToSnsNames:[NSArray arrayWithObjects:UMShareToSina,UMShareToTencent,UMShareToRenren,UMShareToWechatTimeline,UMShareToWechatSession, nil] delegate:self];
//    
//    
//    //1.微信分享
...
- (void)initViews{
    ...
    _activityView.frame = CGRectMake(screen_width*0.5 - 15, screen_height*0.5-15, 30, 30);
...
- (void)getShopData{
    ...
    [[MTNetworkSingletion shareManager]getShopResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"店铺详情获得成功");
...
- (void)getShopRecommendData{
    ...
    [[MTNetworkSingletion shareManager]getShopRecommendResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"店铺详情页面的推荐数据请求成功");

...
- (void)setNavi
{
    ...
    //放大镜
    UIImageView *searchImage = [[UIImageView alloc] initWithFrame:CGRectMake(5, (searchView.height - 15)*0.5, 15, 15)];
    [searchImage setImage:[UIImage imageNamed:@"icon_homepage_search"]];
    [searchView addSubview:searchImage];
    
    //placeholder
    UILabel *placeHolderLable = [[UILabel alloc] initWithFrame:CGRectMake(25, 0, 150, 25)];
    placeHolderLable.font = [UIFont systemFontOfSize:13];
    placeHolderLable.text = @"请输入商家、品类、商圈";
    placeHolderLable.textColor = [UIColor whiteColor];
    [searchView addSubview:placeHolderLable];
...
- (void)getRushBuyData{
    ...
    [[MTNetworkSingletion shareManager] getRushBuyResult:nil url:url success:^(id responseBody) {
        MTLog(@"抢购请求成功");
...
- (void)getHotQueueData{
    ...
    [[MTNetworkSingletion shareManager] getHotQueueResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"获取热门排队：成功");
        NSDictionary *dataDic = [responseBody objectForKey:@"data"];
...
//    MTLog(@"最新的经纬度：%f,%f",delegate.latitude,delegate.longitude);
    
    [[MTNetworkSingletion shareManager] getRecommendResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"推荐数据获得成功");
...
- (void)getDiscountData{
    ...
    [[MTNetworkSingletion shareManager] getDiscountResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"获取折扣数据成功");
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 3){
        ...
        if(indexPath.row != 0){
            RecommendModel *recomend = _recommendArray[indexPath.row - 1];
            NSString *shopId = [recomend.id stringValue];

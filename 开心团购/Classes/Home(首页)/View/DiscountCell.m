...
- (void)setDiscountArray:(NSMutableArray *)discountArray{
    ...
    for(int i = 0;i<discountArray.count;i++){
        DiscountModel *discountModel = discountArray[i];
        NSNumber *num = [[NSNumber alloc] initWithInt:1];
        if([discountModel.type isEqualToValue:num]){
            [array addObject:discountModel];
        }
    }
...
- (void)OnTapBackView:(UITapGestureRecognizer *)tap{ 

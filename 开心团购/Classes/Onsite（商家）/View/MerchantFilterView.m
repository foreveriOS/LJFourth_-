...
@interface MerchantFilterView()<UITableViewDataSource,UITableViewDelegate>
{
    NSMutableArray *_bigGroupArray;//左边的tableView的数据源
...
-(void)getCateListData{
    ...
    [[MTNetworkSingletion shareManager] getCateListResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"获取cate分组信息成功");
        NSMutableArray *dataArray = [responseBody objectForKey:@"data"];
        
        for (int i = 0; i < dataArray.count; i++) {
            MerCateGroupModel *cateM = [MerCateGroupModel objectWithKeyValues:dataArray[i]];

...
@interface JZMerchantViewController ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,MerchantFilterViewDelegate>
{
...
-(void)initData{
    ...
    NSLog(@"_locationInfoStr=%@",_locationInfoStr);
...
- (void)getMerchantList{
    ...
    [[MTNetworkSingletion shareManager] getMerchantListResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"获取商家列表成功");
...
//        NSLog(@"offset:%ld",_offset);
        ...
        for (int i = 0; i < dataArray.count; i++) {
            MerchantModel *JZMerM = [MerchantModel objectWithKeyValues:dataArray[i]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MerchantCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
    if (cell == nil) {
        cell = [[MerchantCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier];
    }
    
    MerchantModel *jzMerM = _merchantArray[indexPath.row];
    cell.jzMerM = jzMerM;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MerchantDetailViewController *jzMerchantDVC = [[MerchantDetailViewController alloc] init];

...
//    [UMSocialSnsService presentSnsIconSheetView:self appKey:UMAPPKEY shareText:@"记得要分享哦" shareImage:[UIImage imageNamed:@"m1.jpg"] shareToSnsNames:[NSArray arrayWithObjects:UMShareToSina,UMShareToTencent,UMShareToRenren,UMShareToWechatTimeline,UMShareToWechatSession, nil] delegate:self];
...
-(void)getMerchantDetailData{
    ...
    [[MTNetworkSingletion shareManager]getMerchantDetailResult:nil url:urlStr success:^(id responseBody) {
        MTLog(@"商家详情请求成功");
        NSMutableArray *dataArray = [responseBody objectForKey:@"data"];
        if (dataArray.count>0) {
            [_dataSourceArray removeAllObjects];
            MerDetailModel *jzMerDetailM = [MerDetailModel objectWithKeyValues:dataArray[0]];
...
-(void)getAroundGroupPurchaseData{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@%@",str1,self.poiid,str2];
    [[MTNetworkSingletion shareManager] getAroundGroupPurchaseResult:nil url:urlStr success:^(id responseBody) {
        ...
        for (int i = 0; i < dealsArray.count; i++) {
            MerAroundGroupModel *jzAroundM = [MerAroundGroupModel objectWithKeyValues:dealsArray[i]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if (_dataSourceArray.count > 0) {
            MerDetailModel *jzMerDM = _dataSourceArray[0];
